...
function Login() {
	//state for CSStranstion
	const [activeForm, setActiveForm] = useState("login");
	//the login form hasnt been completely removed when the register form is now visible so there is a height bug
	//make it dynamic by calculating the height using state
	...
	//hook from redux to dispatch user to data store
	...
	return (
		<div className="login">
			...
			<div className="login__form" style={{ height: formHeight }}>
				{/* the in={} shows the form if the state is true & the unmountOnExit hides it if its not */}
				{/* timeout for the animation duration and classname for transition style */}
				{/* the onEnter is a life cycle hook from CSStranstion which calls a callback as soon as the yourclass-enter is added to the element */}
				<CSSTransition
					in={activeForm === "login"}
					unmountOnExit
					timeout={500}
			...
			<div className="login__footer">
				<div className="loginFooter__logo">
					<svg
						height="16"
						viewBox="0 0 102 26"
						fill="none"
				...
				<p className="loginFooter__creator">
					Created by <span className="name">Joseph Tham 😎</span>
				</p>
			</div>
		</div>
	);
